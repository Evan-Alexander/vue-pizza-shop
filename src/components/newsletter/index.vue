<template>
  <div class="container newsl_container">
    <h2>Subscribe to our newsletter</h2>
    <div class="form">
      <input type="text" v-model="email"/>
      <button @click="submitHandler">Subscribe</button>
    </div>
    <div class="error_label">
      <div>{{ error }}</div>
    </div>
    <div class="success_label">
      <div>{{ success }}</div>
    </div>
    <div class="small">
      Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolorum atque veniam sunt ab cum natus nam dicta magnam. 
    </div>
  </div>
</template>

<script>
  export default {
    data() {
      return {
        email: '',
        error: '',
        success: ''
      }
    },
    methods: {
      validate(email) {
        let valid = [true, ''];

        if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
          valid = [false, 'Enter a valid email']
        }

        if(email === '') {
          valid = [false, 'Please enter your email address']
        }
        return valid;
      },
      submitHandler() {
        let  valid = this.validate(this.email);
        // If valid (first position equals true ...)
        if(valid[0]) {
          // clear error message
          this.error = '';
          this.addEmail(this.email)
        } else {
          this.error = valid[1]
        }
      },
      addEmail(email) {
        /* eslint-disable */
        this.$http.get(`users.json?orderBy=\"email\"&&equalTo=\"${email}\"`)
        /* eslint-enable */
        .then(response => {
          if(Object.getOwnPropertyNames(response.data).length === 0) {
            this.$http.post('users.json', { email: this.email })
            /* eslint-disable */
            .then(response => {
                this.success = 'Thank You for subscribing!'  
              /* eslint-enable */
            })
          } else {
            this.success = 'Already on the list'
          }
        });
        this.clearSuccess()     
      },
      clearSuccess() {
        setTimeout(() => {
          this.email = '';
          this.success = '';
        }, 2000)
      }
    }
  }
</script>